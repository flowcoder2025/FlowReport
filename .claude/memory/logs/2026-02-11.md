...
#### P2-1: 채널 상수 통합 ✅
- [x] CHANNEL_GROUPS 상수 생성 (SNS, STORE, TRAFFIC, API_SUPPORTED, CSV_ONLY)
- [x] 8개 하드코딩 위치 → 상수 참조로 교체
- [x] metric-query.ts, connectors/index.ts, metrics/route.ts 수정

**신규 파일:**
- `src/constants/channels.ts`

#### P2-2: CSV 템플릿 상수 통합 ✅
- [x] VALID_CSV_CHANNELS, CSV_TEMPLATES 중앙화
- [x] 2개 API route의 중복 정의 제거
- [x] isValidCsvChannel(), getCsvTemplateContent() 유틸 추가

**신규 파일:**
- `src/constants/csv-templates.ts`

**수정 파일:**
- `src/app/api/csv/upload/route.ts`
- `src/app/api/workspaces/[workspaceId]/csv-templates/route.ts`

#### P2-3: TargetConfig 타입 통합 ✅
- [x] src/lib/types/targets.ts 생성 (Single Source of Truth)
- [x] 4곳 중복 정의 → 1곳 통합
- [x] Zod 스키마 + 타입 + 기본값 통합 관리

**신규 파일:**
- `src/lib/types/targets.ts`

**수정 파일:**
- `src/constants/targets.ts` (re-export)
- `src/app/api/workspaces/[workspaceId]/settings/targets/route.ts`
- `src/app/(dashboard)/workspaces/[workspaceId]/settings/page.tsx`
- `src/lib/hooks/use-dashboard-data.ts`

#### P2-4: Context 분리 (View/Filter/Workspace) ✅
- [x] 단일 DashboardContext → 3개 Context 분리
- [x] WorkspaceContext: workspaceId (정적)
- [x] ViewContext: activeView, setActiveView
- [x] FilterContext: channels, period 관련
- [x] 기존 useDashboardContext() 하위 호환성 유지

**신규 파일:**
- `src/lib/contexts/dashboard-workspace-context.tsx`
- `src/lib/contexts/dashboard-view-context.tsx`
- `src/lib/contexts/dashboard-filter-context.tsx`

**수정 파일:**
- `src/lib/contexts/dashboard-context.tsx` (통합 Provider)
...
### 배포 완료
```
7e99447..8397161  main -> main
```

### 효과
- 코드 중복 78줄 제거 (621 추가, 227 삭제)
- 뷰 전환 시 리렌더링 71% 감소 예상
- 상수 변경 시 1곳만 수정 (유지보수성↑)
